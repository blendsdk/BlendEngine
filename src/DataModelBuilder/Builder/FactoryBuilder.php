...
class FactoryBuilder extends ClassBuilder
{
    ...
    protected function preparBuildDefinition($def)
    {
        ...
        $def['uniqueKeys'] = array_merge(
                $this->createDefinitionsForKeys($this->relation->getUniqueKeys()), $this->createDefinitionsForKeys($this->relation->getCustomKeys('CUSTOM_SINGLE'))
        );
        $def['multiKeys'] = array_merge(
                $this->createDefinitionsForKeys($this->relation->getForeignKeys()), $this->createDefinitionsForKeys($this->relation->getCustomKeys('CUSTOM_MULTI'))
        );
